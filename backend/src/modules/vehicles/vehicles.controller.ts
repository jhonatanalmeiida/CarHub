  ...
  try {
    ...
    const where = {
      ...(query.search
        ? {
            OR: [
              { trim: { contains: query.search, mode: "insensitive" as const } },
              { model: { name: { contains: query.search, mode: "insensitive" as const } } },
              { model: { brand: { name: { contains: query.search, mode: "insensitive" as const } } } }
            ]
          }
        : {}),
    ...
    const [items, total] = await Promise.all([
      prisma.vehicle.findMany({
        ...
        orderBy: { [query.sortBy || "createdAt"]: query.sortOrder || "desc" } as any,
        ...pagination
      }),
...
vehiclesRouter.post("/", authGuard, roleGuard([ROLES.ADMIN]), validate(vehicleMutationSchema), async (req, res, next) => {
  try {
    const item = await prisma.vehicle.create({ data: req.body as any });
