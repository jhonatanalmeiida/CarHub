...
comparisonsRouter.post("/", validate(comparisonCreateSchema), async (req, res, next) => {
    try {
        const comparison = await prisma.comparison.create({
            data: {
                userId: req.user.sub,
                items: {
                    create: req.body.vehicleIds.map((vehicleId) => ({ vehicleId }))
                }
            },
            include: {
                items: {
...
comparisonsRouter.delete("/:id", async (req, res, next) => {
    try {
